...
public class UnderwaterMine : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D WhoCollidedWithMe)
	{
		if (WhoCollidedWithMe.tag == "Bullet")
		{
            ...
            GameObject mineExplosion = (GameObject)GameObject.Instantiate(MineExplosionParticle, new Vector3(this.transform.position.x, this.transform.position.y, -5), Quaternion.identity);
